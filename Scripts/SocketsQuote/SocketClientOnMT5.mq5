...
void OnStart()
{
   ...
   socket=SocketCreate();
   int socket_statu = SocketConnect(socket,InpIP,InpPort,1000);
   //监测ticks变动
   while(!_StopFlag)
   {  
         ...
         {  
            ...
            {  
               if(socket_statu)
               {
                  ...
                  string sendbuf_tick = IntegerToString(StringLen(tosend_tick),4) + tosend_tick;    
